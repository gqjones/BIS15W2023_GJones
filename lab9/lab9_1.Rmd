...
+ `categorical` qualitative data that can take on a limited number of non-numerical values
...
```{r}
ggplot(homerange, mapping=aes(log10.mass, log10.hra))+
  geom_point()
```
...
```{r}
ggplot(homerange, mapping=aes(log10.hra, log10.preymass))+
  geom_point()+
  geom_smooth(method=lm, se=T)
```
...
mammals%>%
  ...
  ggplot(aes(common.name, log10.mass))+
  geom_col()
```
