...
## 1     170566
```
When summarizing the number of NAs in amniota and amphibio data, we see that amniota contains no expicit NAs. However we know that amniota represents NAs as "-999".
...
## 3 Reptilia     6040
```
Biologically, it does not make sense for a taxonomic class to have an egg mass of NA. The NA here may be representing a lack of data or information for the specific species. 

**9. The `amphibio` data have variables that classify species as fossorial (burrowing), terrestrial, aquatic, or arboreal.Calculate the number of NA's in each of these variables. Do you think that the authors intend us to think that there are NA's in these columns or could they represent something else? Explain.**

```r
amphibio%>%
  summarize(across(c(Fos, Ter, Aqu, Arb), is.na))%>%
  summarize(across(c(Fos, Ter, Aqu, Arb), sum))
```
...
```
The NAs in these columns may indicate that some species are unable to be classified as fossorial, terrestrial, aquatic, or aboreal. This may be due to a lack of information on that species or inconclusive information on the species. 
