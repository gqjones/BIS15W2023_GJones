...
```{r}
pivot_longer(heartrate, -patient, names_to = "drug", values_to = "heartrate")
```
...
Because the columns names are a peice of data. i.e the income range is a piece of data
3. Use `pivot_longer()` to make the data tidy.  
```{r}
relig_income%>%
  pivot_longer(-religion, names_to = "income_range", values_to = "n")
```
...
```{r}
plant_data <- read.csv(here("lab8", "data", "plant_data.csv"))
```

2. Why are these data not tidy?  

Each day is its own observation. They should be listed in a column.
