...
expenditures <- expenditures%>%
  pivot_longer(-expenditure, names_to = "year", values_to = "value")
```
...
heartrate3 <- heartrate2%>%
  separate(patient, into = c("patient", "sex"), sep="_")%>%
  pivot_longer(c(a, b ,c ,d), names_to = "drug", values_to = "heartrate")%>%
  arrange(-heartrate)
heartrate3
```
...
```{r}
unite(heartrate3, "patient", "sex", sep=" ")
```
...
```{r}
beachbugs <- beachbugs%>%
  pivot_wider(names_from = "site", values_from = "buglevels", names_sep = "_")
```
