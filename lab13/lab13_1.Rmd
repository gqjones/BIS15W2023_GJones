...
## Accessing Data Frames in a List
```{r}
spider_list$Kern
```
...
`inner_join`  
Where did customers 3 and 4 go? inner_join() returns a data frame with all the data from both data frames that contain the same customer_ID
```{r}
...
`left_join`
Where did customer 4 go? Notice the missing data for customer 3. left_join() is an inner join but keeps all the data from the first data frame
```{r}
...
`right_join`
Where did customer 3 go? Notice the missing data for customer 4. right_join() is an inner join but keeps all the data from the second data frame
```{r}
...
`full_join`
full_join() returns a combination of the two data frames regardless of matches/mismatches
```{r}
